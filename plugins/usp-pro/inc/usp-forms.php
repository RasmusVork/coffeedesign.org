...
function usp_form_wrap($args, $success) {
	...
	if ($args) {
		...
		if (usp_is_contact($form_id)) $contact_form = '<input name="usp-is-contact" value="1" type="hidden" />'. "\n";
		else $contact_form = '';
		...
		if ($current_user->ID) $user_login = '<input name="usp-logged-id" value="' . $current_user->ID . '" type="hidden" />'. "\n";
		else $user_login = '';
		...
		if (!empty($usp_general['use_cat']) && !empty($usp_general['use_cat_id'])) {
			...
			$user_cat = '<input name="usp-logged-cat" value="'. $value .'" type="hidden" />'. "\n";
		} else { 
			$user_cat = '';
		}
		...
		$session_verify = '<input name="PHPSESSID" value="'. session_id() .'" type="hidden" />'. "\n";
		$user_verify = '<input name="usp-verify" id="verify" type="text" value="" style="display:none;" />'. "\n";
