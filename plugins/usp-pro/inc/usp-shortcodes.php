...
function usp_input_name($args) {
	...
	$content .= '<input name="usp-name" type="text" value="'. $value .'" data-required="'. $required .'" '. $parsley .'maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
...
function usp_input_url($args) {
	...
	$content .= '<input name="usp-url" type="text" value="'. $value .'" data-required="'. $required .'" '. $parsley .'maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
...
function usp_input_title($args) {
	...
	$content .= '<input name="usp-title" type="text" value="'. $value .'" data-required="'. $required .'" '. $parsley .'maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
...
function usp_input_captcha($args) {
	...
	if ((isset($recaptcha_public) && !empty($recaptcha_public)) && (isset($recaptcha_private) && !empty($recaptcha_private))) {
		...
		$captcha = '<input name="usp-captcha" type="text" value="'. $value .'" data-required="true" required="required" maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
	}
	if (empty($label)) $content = '';
	...
	if (isset($usp_general['hidden_cats']) && !empty($usp_general['hidden_cats'])) {
		...
		if ($display_cats == 'checkbox') {
			...
			foreach ($categories as $cat) {
				...
				$content .= '<span class="usp-checkbox usp-cat usp-cat-'. $level .'"><input type="checkbox" name="usp-category[]" value="'. $cat['id'] .'" data-required="'. $required .'" class="'. $classes .'"'. $checked .' /> '. get_cat_name($cat['id']) .'</span>' . "\n";
			}
		} else {
			if (empty($label)) $content = '';
		...
		if ($required == 'true') $content .= '<input name="usp-category-required" value="1" type="hidden" />'. "\n";
		if (!empty($cats)) $content .= '<input name="usp-cats-default" value="'. $cats .'" type="hidden" />'. "\n";
		return $fieldset_before . $content . $fieldset_after;
	}
}
	...
	if (!empty($tax_terms)) {
		if ($type == 'checkbox') {
			...
			foreach ($tax_terms as $tax) {
				...
				$content .= '<span class="usp-checkbox usp-tax"><input type="checkbox" name="usp-taxonomy-'. $taxonomy .'[]" value="'. $tax['term_id'] .'" data-required="'. $required .'" class="'. $classes .'"'. $checked .' /> '. $tax['name'] .'</span>' . "\n";
			}
		} else {
			if (empty($label)) $content = '';
		...
		if ($multiple) {
			...
			if (empty($method)) {
				for ( $i = 1; $i <= $number_files; $i++ ) {
					$content .= '<input name="usp-files[]" type="file" maxlength="'. $max .'" data-required="'. $required .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
				}
				$content .= '<div class="usp-add-another"><a href="#">'. $link .'</a></div>'. "\n";
			} else {
				$content .= '<input name="usp-files[]" type="file" maxlength="'. $max .'" data-required="'. $required .'" placeholder="'. $placeholder .'" class="'. $classes .'"'. $method .' id="usp-multiple-files" />'. "\n";
				$content .= '<div class="usp-preview"></div>'. "\n";
			}
			$content .= '</div>'. "\n";
			$content .= '<input name="usp-file-limit" id="usp-file-limit" value="'. $usp_uploads['max_files'] .'" type="hidden" />'. "\n";
			$content .= '<input name="usp-file-count" id="usp-file-count" value="1" type="hidden" />'. "\n";
			if ($required == 'true') $content .= '<input name="usp-files-required" value="1" type="hidden" />'. "\n";
			...
			$content .= '<input name="usp-file-'. $key .'" type="file" maxlength="'. $max .'" data-required="'. $required .'" placeholder="'. $placeholder .'" class="'. $classes .' usp-input-file usp-input-file-'. $key .'" />'. "\n";
			$content .= '<input name="usp-file-key" value="'. $key .'" type="hidden" />'. "\n";
			if ($required == 'true') $content .= '<input name="usp-file-required-'. $key .'" value="1" type="hidden" />'. "\n";
			if (!empty($file_types)) $content .= '<input name="usp-file-types" value="'. $file_types .'" type="hidden" />'. "\n";
			$content .= '</div>'. "\n";
		}
	} else {
...
function usp_remember($args) {
	...
	$content = '';
	$content .= '<input name="usp-remember" id="usp-remember" type="checkbox" data-required="true" class="'. $classes .'" value="1"'. $checked .' /> '. "\n". $label;
...
function usp_submit_button($args) {
	...
	return $fieldset_before . '<input type="submit" class="'. $classes .'" value="'. $value .'" />'. "\n" . $fieldset_after;
}
...
function usp_input_email($args) {
	...
	$content .= '<input name="usp-email" type="text" value="'. $value .'" data-required="'. $required .'" '. $parsley .'maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
...
function usp_input_subject($args) {
	...
	$content .= '<input name="usp-subject" type="text" value="'. $value .'" data-required="'. $required .'" '. $parsley .'maxlength="'. $max .'" placeholder="'. $placeholder .'" class="'. $classes .'" />'. "\n";
